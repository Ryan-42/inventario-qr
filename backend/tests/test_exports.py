...
def _export(client, sid, _tok, path):
    return client.post(f"/api/sessoes/{sid}/exportar/{path}")
...
def test_export_excel_completo(client, sessao_com_itens):
    ...
    r = _export(client, sid, None, "completo")
...
def test_export_divergencias(client, sessao_com_itens):
    ...
    r = _export(client, sid, None, "divergencias")
    assert r.status_code == 200
    assert "spreadsheetml" in r.headers["content-type"]
...
def test_export_pdf(client, sessao_com_itens):
    ...
    r = _export(client, sid, None, "pdf")
    assert r.status_code == 200
    assert r.headers["content-type"] == "application/pdf"
    assert r.content[:4] == b"%PDF"


def test_export_sem_jwt_retorna_401(client, sessao_com_itens):
    sid = sessao_com_itens["id"]
    r = client.post(f"/api/sessoes/{sid}/exportar/completo",
                    headers={"Authorization": "Bearer invalido"})
    assert r.status_code == 401


def test_export_sessao_inexistente(client):
    r = client.post("/api/sessoes/nao-existe/exportar/completo")
    assert r.status_code == 404


def test_export_etiquetas(client, sessao_com_itens):
    sid = sessao_com_itens["id"]
    r = _export(client, sid, None, "etiquetas")
    assert r.status_code == 200
    assert r.headers["content-type"] == "application/pdf"
    assert r.content[:4] == b"%PDF"
...
def test_export_etiquetas_sem_itens(client, sessao):
    sid = sessao["id"]
    r = client.post(f"/api/sessoes/{sid}/exportar/etiquetas")
    assert r.status_code == 422
    assert "Nenhum item" in r.json()["detail"]


def test_export_etiquetas_sessao_inexistente(client):
    r = client.post("/api/sessoes/nao-existe/exportar/etiquetas")
    assert r.status_code == 404
...
def test_export_relatorio_final_pdf_basico(client, sessao_com_itens):
    ...
    r = _export(client, sid, None, "relatorio-final-pdf")
...
def test_export_relatorio_final_excel_basico(client, sessao_com_itens):
    ...
    r = _export(client, sid, None, "relatorio-final-excel")
    assert r.status_code == 200, r.text
    assert "spreadsheetml" in r.headers["content-type"]
