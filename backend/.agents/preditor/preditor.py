...
class PredictionAgent:
    ...
    def prever(self, sessao_id: str, db) -> dict:
        from sqlalchemy import func  # noqa: F401 — disponível para queries futuras
        ...
        sessoes_anteriores = (
            db.query(Sessao)
            .filter(Sessao.id != sessao_id, Sessao.status == StatusSessao.concluida)
        ...
        hist_stats = []
        codigos_historico_problemas: dict[str, int] = {}
        ...
        for item in itens_atuais:
            ...
            if fator_risco > 0 or (valor > 5000.0 and len(sessoes_anteriores) > 0):
                ...
                itens_alto_risco_local.append({
                    ...
                    "motivo": (
                        f"Divergiu em {fator_risco} inventário(s) anterior(es)."
                        if fator_risco > 0
                        else "Item de alto valor com histórico pendente."
                    ),
                })

        itens_alto_risco_local = sorted(
            itens_alto_risco_local, key=lambda x: x["probabilidade_divergencia_pct"], reverse=True
        )[:15]

        est_tempo_minutos = max(30, int(total_itens_novos * 1.2))
...
{json.dumps(hist_stats, ensure_ascii=False)}

ITENS COM MAIOR RISCO NO HISTÓRICO:
{json.dumps(itens_alto_risco_local[:10], ensure_ascii=False)}
...
{{
  ...
  "recomendacoes": [
    "recomendação operacional específica para prevenir divergências"
  ]
}}
"""
